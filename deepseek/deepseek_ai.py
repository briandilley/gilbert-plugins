"""DeepSeek AI backend — AI via the DeepSeek OpenAI-compatible API.

DeepSeek exposes an OpenAI-compatible Chat Completions endpoint at
``https://api.deepseek.com/v1/chat/completions`` that accepts the same
request shape, streaming protocol, and tool-calling format as OpenAI.
We speak it directly over ``httpx`` — no ``openai`` or ``deepseek`` SDK.
"""
...
_AVAILABLE_MODELS = [
    ModelInfo(
        id="deepseek-chat",
        name="DeepSeek V3",
        description="General-purpose flagship — strong reasoning and tool use.",
    ),
    ModelInfo(
        id="deepseek-reasoner",
        name="DeepSeek R1",
        description="Reasoning-focused model — stronger for math, code, and planning.",
    ),
]


class DeepSeekAI(AIBackend):
    """AI backend using DeepSeek's OpenAI-compatible Chat Completions API."""
    ...
    def backend_config_params(cls) -> list[ConfigParam]:
        ...
        return [
            ...
            ConfigParam(
                ...
                type=ToolParameterType.STRING,
                description=(
                    "API base URL. Leave at the default for DeepSeek's hosted "
            ...
            ConfigParam(
                ...
                description=(
                    ...
                    "``max_tokens`` to the DeepSeek endpoint."
                ),
                default=8192,
            ),
            ConfigParam(
    ...
    def backend_actions(cls) -> list[ConfigAction]:
        return [
            ConfigAction(
                ...
                description=(
                    "Send a tiny 'hi' message to the DeepSeek API to verify the API key and model."
                ),
            ),
        ]
    ...
    async def _action_test_connection(self) -> ConfigActionResult:
        if self._client is None:
            return ConfigActionResult(
                status="error",
                message="DeepSeek backend is not initialized — save settings first.",
            )
        try:
            request = AIRequest(
        ...
        return ConfigActionResult(
            status="ok",
            message=f"Connected to DeepSeek (model: {response.model}).",
        )
    ...
    async def initialize(self, config: dict[str, Any]) -> None:
        ...
        if not api_key:
            raise ValueError("DeepSeekAI requires 'api_key' in config")
        ...
        logger.info("DeepSeek AI backend initialized (model=%s)", self._model)
    ...
    async def generate(self, request: AIRequest) -> AIResponse:
        if self._client is None:
            raise RuntimeError("DeepSeekAI not initialized")
        ...
        ai_logger.debug(
            "DeepSeek response: finish_reason=%s usage=%s",
    ...
    async def generate_stream(
        ...
        """Stream DeepSeek SSE chunks as provider-neutral ``StreamEvent``s.

        DeepSeek's endpoint emits OpenAI-shape SSE frames, so parsing
        ...
        ai_logger.debug(
            "DeepSeek stream request: model=%s messages=%d",
            self._model,
            len(body["messages"]),
        )
        ...
        ai_logger.debug(
            "DeepSeek stream response: finish_reason=%s usage=%s",
            finish_reason_raw,
    ...
    def _build_messages(
        ...
        for msg in messages:
            ...
            if msg.role == MessageRole.USER:
                ...
                # DeepSeek (like OpenAI) requires the tool results to
    ...
    def _build_user_message(self, msg: Message) -> dict[str, Any]:
        ...
        DeepSeek's current chat models don't accept native image
    ...
    def _error_from_response(
        ...
        try:
            ...
            err_body = resp.text
        ai_logger.warning(
            "DeepSeek API error: status=%d body=%s request=%s",
            status,
            err_body,
        ...
        return AIBackendError(
            f"DeepSeek API rejected request ({status}): {reason}",
            status=status,
        )
    ...
    def _error_from_stream_body(
        ...
        ai_logger.warning(
            "DeepSeek stream API error: status=%d body=%s",
            status,
            err_body,
        )
        reason = self._extract_error_reason(err_body)
        return AIBackendError(
            f"DeepSeek API rejected streaming request ({status}): {reason}",
            status=status,
        )
