"""xAI (Grok) AI backend — AI via xAI's OpenAI-compatible API.

xAI exposes an OpenAI-compatible Chat Completions endpoint at
``https://api.x.ai/v1/chat/completions`` that accepts the same request
shape, streaming protocol, and tool-calling format as OpenAI. Runs the
Grok family of models. We speak it directly over ``httpx``.
"""
...
_AVAILABLE_MODELS = [
    ...
    ModelInfo(
        id="grok-3",
        name="Grok 3",
        description="Previous-generation flagship — balanced quality and cost.",
    ),
    ModelInfo(
        id="grok-3-mini",
        name="Grok 3 Mini",
    ...
    ModelInfo(
        ...
        name="Grok 2 Vision",
        description="Multimodal model — image understanding plus chat.",
    ),
    ModelInfo(
        id="grok-2-1212",
        name="Grok 2",
        description="Text-only chat model from the Grok 2 generation.",
    ),
]


class XAIAI(AIBackend):
    """AI backend using xAI's OpenAI-compatible Chat Completions API."""
    ...
    def backend_config_params(cls) -> list[ConfigParam]:
        ...
        return [
            ...
            ConfigParam(
                ...
                type=ToolParameterType.STRING,
                description=(
                    "API base URL. Leave at the default for xAI's hosted "
            ...
            ConfigParam(
                ...
                description=(
                    ...
                    "``max_tokens`` to the xAI endpoint."
                ),
                default=8192,
            ),
            ConfigParam(
    ...
    def backend_actions(cls) -> list[ConfigAction]:
        return [
            ConfigAction(
                ...
                description=(
                    "Send a tiny 'hi' message to the xAI API to verify the API key and model."
                ),
            ),
        ]
    ...
    async def _action_test_connection(self) -> ConfigActionResult:
        if self._client is None:
            return ConfigActionResult(
                status="error",
                message="xAI backend is not initialized — save settings first.",
            )
        try:
            request = AIRequest(
        ...
        return ConfigActionResult(
            status="ok",
            message=f"Connected to xAI (model: {response.model}).",
        )
    ...
    async def initialize(self, config: dict[str, Any]) -> None:
        ...
        if not api_key:
            raise ValueError("XAIAI requires 'api_key' in config")
        ...
        logger.info("xAI AI backend initialized (model=%s)", self._model)
    ...
    async def generate(self, request: AIRequest) -> AIResponse:
        if self._client is None:
            raise RuntimeError("XAIAI not initialized")

        body = self._build_request_body(request)

        ai_logger.debug(
            "xAI request: model=%s messages=%d",
            body["model"],
            len(body["messages"]),
        )
        ...
        ai_logger.debug(
            "xAI response: finish_reason=%s usage=%s",
    ...
    async def generate_stream(
        ...
        """Stream xAI SSE chunks as provider-neutral ``StreamEvent``s.

        xAI's endpoint emits OpenAI-shape SSE frames, so parsing
        ...
        ai_logger.debug(
            "xAI stream request: model=%s messages=%d",
            self._model,
            len(body["messages"]),
        )
        ...
        ai_logger.debug(
            "xAI stream response: finish_reason=%s usage=%s",
            finish_reason_raw,
    ...
    def _build_messages(
        ...
        for msg in messages:
            ...
            if msg.role == MessageRole.USER:
                ...
                # xAI (like OpenAI) requires the tool results to
    ...
    def _build_user_message(self, msg: Message) -> dict[str, Any]:
        ...
        Grok 2 Vision accepts ``image_url`` content parts with base64
        data URLs (same shape as OpenAI). Non-vision Grok models ignore
        vision parts, so sending the data URL is safe for all models.
        """
    ...
    def _error_from_response(
        ...
        try:
            ...
            err_body = resp.text
        ai_logger.warning(
            "xAI API error: status=%d body=%s request=%s",
            status,
            err_body,
        ...
        return AIBackendError(
            f"xAI API rejected request ({status}): {reason}",
            status=status,
        )
    ...
    def _error_from_stream_body(
        ...
        ai_logger.warning(
            "xAI stream API error: status=%d body=%s",
            status,
            err_body,
        )
        reason = self._extract_error_reason(err_body)
        return AIBackendError(
            f"xAI API rejected streaming request ({status}): {reason}",
            status=status,
        )
