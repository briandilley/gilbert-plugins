"""Tests for xAI (Grok) backend — vendor defaults + core contract."""

from unittest.mock import AsyncMock, MagicMock

import pytest
from gilbert_plugin_xai.xai_ai import XAIAI
...
async def test_initialize_requires_api_key(backend: XAIAI) -> None:
    with pytest.raises(ValueError, match="api_key"):
        await backend.initialize({})


async def test_default_base_url_is_xai(backend: XAIAI) -> None:
    await backend.initialize({"api_key": "xai-test"})
    assert backend._client is not None
    resolved = str(backend._client.build_request("POST", "/chat/completions").url)
    assert resolved == "https://api.x.ai/v1/chat/completions"
    await backend.close()


async def test_default_model_is_grok_4(backend: XAIAI) -> None:
    await backend.initialize({"api_key": "xai-test"})
    assert backend._model == "grok-4-0709"
    await backend.close()


def test_capabilities_reports_streaming_and_attachments() -> None:
    caps = XAIAI().capabilities()
    assert caps.streaming is True
    assert caps.attachments_user is True


def test_backend_name() -> None:
    assert XAIAI.backend_name == "xai"


def test_backend_config_params_includes_enabled_toggle() -> None:
    params = XAIAI.backend_config_params()
...
def test_build_request_body_uses_max_tokens() -> None:
    ...
    assert body["model"] == "grok-4-0709"
    assert body["max_tokens"] == 256
    assert body["temperature"] == 0.3


def test_build_messages_image_uses_image_url_part() -> None:
    """Grok 2 Vision accepts image_url content parts — attachments with
    data are inlined as OpenAI-shape data URLs."""
    backend = XAIAI()
    messages = [
        Message(
...
async def test_generate_calls_chat_completions(backend: XAIAI) -> None:
    await backend.initialize({"api_key": "xai-test"})
...
async def test_generate_raises_ai_backend_error(backend: XAIAI) -> None:
    await backend.initialize({"api_key": "xai-test"})
    mock_response = MagicMock()
...
async def test_generate_raises_when_not_initialized(backend: XAIAI) -> None:
