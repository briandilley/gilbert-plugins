"""Mistral AI backend — AI via the Mistral La Plateforme API.

Mistral's La Plateforme exposes an OpenAI-compatible Chat Completions
endpoint at ``https://api.mistral.ai/v1/chat/completions`` that accepts
the same request shape, streaming protocol, and tool-calling format as
OpenAI. We speak it directly over ``httpx`` — no ``mistralai`` SDK.
"""
...
_AVAILABLE_MODELS = [
    ModelInfo(
        ...
        name="Mistral Large",
        description="Flagship reasoning model — strongest tool use and coding.",
    ),
    ModelInfo(
        id="mistral-medium-latest",
        name="Mistral Medium",
        description="Mid-tier balanced model — quality near Large at lower cost.",
    ),
    ModelInfo(
        id="mistral-small-latest",
        name="Mistral Small",
        description="Compact chat model — fast and inexpensive.",
    ),
    ModelInfo(
        id="codestral-latest",
        name="Codestral",
        description="Code-specialized model — strong at code generation and fill-in.",
    ),
    ModelInfo(
        id="open-mistral-nemo",
        name="Mistral Nemo",
        description="Open-weight 12B model — cheap, decent quality, Apache 2.0.",
    ),
    ModelInfo(
        id="pixtral-large-latest",
        name="Pixtral Large",
        description="Multimodal flagship — image understanding plus chat.",
    ),
]


class MistralAI(AIBackend):
    """AI backend using Mistral's OpenAI-compatible Chat Completions API."""
    ...
    def backend_config_params(cls) -> list[ConfigParam]:
        ...
        return [
            ...
            ConfigParam(
                ...
                type=ToolParameterType.STRING,
                description=(
                    "API base URL. Leave at the default for Mistral's hosted "
            ...
            ConfigParam(
                ...
                description=(
                    ...
                    "``max_tokens`` to the Mistral endpoint."
                ),
                default=8192,
            ),
            ConfigParam(
    ...
    def backend_actions(cls) -> list[ConfigAction]:
        return [
            ConfigAction(
                ...
                description=(
                    "Send a tiny 'hi' message to the Mistral API to verify the API key and model."
                ),
            ),
        ]
    ...
    async def _action_test_connection(self) -> ConfigActionResult:
        if self._client is None:
            return ConfigActionResult(
                status="error",
                message="Mistral backend is not initialized — save settings first.",
            )
        try:
            request = AIRequest(
            ...
            return ConfigActionResult(
                status="error",
                message=f"Mistral API error: {exc}",
            )
        ...
        return ConfigActionResult(
            status="ok",
            message=f"Connected to Mistral (model: {response.model}).",
        )
    ...
    async def initialize(self, config: dict[str, Any]) -> None:
        ...
        if not api_key:
            raise ValueError("MistralAI requires 'api_key' in config")
        ...
        logger.info("Mistral AI backend initialized (model=%s)", self._model)
    ...
    async def generate(self, request: AIRequest) -> AIResponse:
        if self._client is None:
            raise RuntimeError("MistralAI not initialized")

        body = self._build_request_body(request)

        ai_logger.debug(
            "Mistral request: model=%s messages=%d",
            body["model"],
            len(body["messages"]),
        )
        ...
        ai_logger.debug(
            "Mistral response: finish_reason=%s usage=%s",
    ...
    async def generate_stream(
        ...
        """Stream Mistral SSE chunks as provider-neutral ``StreamEvent``s.

        Mistral's endpoint emits OpenAI-shape SSE frames, so parsing
        ...
        ai_logger.debug(
            "Mistral stream request: model=%s messages=%d",
            self._model,
            len(body["messages"]),
        )
        ...
        ai_logger.debug(
            "Mistral stream response: finish_reason=%s usage=%s",
            finish_reason_raw,
    ...
    def _build_messages(
        ...
        for msg in messages:
            ...
            if msg.role == MessageRole.USER:
                ...
                # Mistral (like OpenAI) requires the tool results to
    ...
    def _build_user_message(self, msg: Message) -> dict[str, Any]:
        ...
        Pixtral models accept ``image_url`` content parts with base64
        data URLs (same shape as OpenAI). Non-vision models receive
        the image as a text stub since Mistral's API ignores vision
        parts when the selected model doesn't support them.
        """
    ...
    def _error_from_response(
        ...
        try:
            ...
            err_body = resp.text
        ai_logger.warning(
            "Mistral API error: status=%d body=%s request=%s",
            status,
            err_body,
        ...
        return AIBackendError(
            f"Mistral API rejected request ({status}): {reason}",
            status=status,
        )
    ...
    def _error_from_stream_body(
        ...
        ai_logger.warning(
            "Mistral stream API error: status=%d body=%s",
            status,
            err_body,
        )
        reason = self._extract_error_reason(err_body)
        return AIBackendError(
            f"Mistral API rejected streaming request ({status}): {reason}",
            status=status,
        )
