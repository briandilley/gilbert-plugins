"""Gemini AI backend — AI via Google's OpenAI-compatible endpoint.

Google exposes an OpenAI-compatible Chat Completions shim in front of
the Gemini models at
``https://generativelanguage.googleapis.com/v1beta/openai/chat/completions``.
It accepts the same request/streaming/tool-calling format as OpenAI,
letting us plug Gemini into Gilbert's agentic loop without touching
Google's native ``generateContent`` wire format.

Gemini models have very large context windows (~1M tokens for 2.5 Pro)
and native multimodal input. Vision-capable models accept base64 data
URLs as ``image_url`` parts on the compat endpoint.
"""
...
_AVAILABLE_MODELS = [
    ModelInfo(
        id="gemini-2.5-pro",
        name="Gemini 2.5 Pro",
        description="Flagship — strongest reasoning, ~1M-token context, multimodal.",
    ),
    ModelInfo(
        id="gemini-2.5-flash",
        name="Gemini 2.5 Flash",
        description="Fast tier — balanced quality and latency, multimodal.",
    ),
    ModelInfo(
        id="gemini-2.5-flash-lite",
        name="Gemini 2.5 Flash Lite",
        description="Smallest 2.5 tier — cheapest, lowest latency.",
    ),
    ModelInfo(
        id="gemini-2.0-flash",
        name="Gemini 2.0 Flash",
        description="Previous-generation fast tier — still multimodal.",
    ),
    ModelInfo(
        id="gemini-1.5-pro",
        name="Gemini 1.5 Pro",
        description="Older flagship — kept for long-context workloads.",
    ),
]


class GeminiAI(AIBackend):
    """AI backend using Gemini's OpenAI-compatible Chat Completions API."""
    ...
    def backend_config_params(cls) -> list[ConfigParam]:
        ...
        return [
            ...
            ConfigParam(
                ...
                description=(
                    "Google AI Studio API key (``AIza…``). Generate one at "
                    "https://aistudio.google.com/apikey — this is not the "
                    "same as a Google Cloud Vertex AI key."
                ),
                sensitive=True,
            ),
            ConfigParam(
                ...
                type=ToolParameterType.STRING,
                description=(
                    "API base URL. Leave at the default for Gemini's hosted "
            ...
            ConfigParam(
                ...
                description=(
                    ...
                    "``max_tokens`` to the Gemini endpoint."
                ),
                default=8192,
            ),
            ConfigParam(
    ...
    def backend_actions(cls) -> list[ConfigAction]:
        return [
            ConfigAction(
                ...
                description=(
                    "Send a tiny 'hi' message to the Gemini API to verify the API key and model."
                ),
            ),
        ]
    ...
    async def _action_test_connection(self) -> ConfigActionResult:
        if self._client is None:
            return ConfigActionResult(
                status="error",
                message="Gemini backend is not initialized — save settings first.",
            )
        try:
            request = AIRequest(
        ...
        return ConfigActionResult(
            status="ok",
            message=f"Connected to Gemini (model: {response.model}).",
        )
    ...
    async def initialize(self, config: dict[str, Any]) -> None:
        ...
        if not api_key:
            raise ValueError("GeminiAI requires 'api_key' in config")
        ...
        logger.info("Gemini AI backend initialized (model=%s)", self._model)
    ...
    async def generate(self, request: AIRequest) -> AIResponse:
        if self._client is None:
            raise RuntimeError("GeminiAI not initialized")
    ...
    async def generate_stream(
        ...
        """Stream Gemini SSE chunks as provider-neutral ``StreamEvent``s.

        Gemini's endpoint emits OpenAI-shape SSE frames, so parsing
        ...
        ai_logger.debug(
            "Gemini stream request: model=%s messages=%d",
            self._model,
            len(body["messages"]),
        )
        ...
        ai_logger.debug(
            "Gemini stream response: finish_reason=%s usage=%s",
            finish_reason_raw,
    ...
    def _build_messages(
        ...
        for msg in messages:
            ...
            if msg.role == MessageRole.USER:
                ...
                # Gemini (like OpenAI) requires the tool results to
    ...
    def _build_user_message(self, msg: Message) -> dict[str, Any]:
        ...
        Gemini models are natively multimodal — the OpenAI-compat
        endpoint accepts ``image_url`` content parts with base64 data
        URLs. Sending them works for every model in the current lineup
        (all 2.5 and 2.0 tiers are multimodal).
        """
    ...
    def _error_from_response(
        ...
        try:
            ...
            err_body = resp.text
        ai_logger.warning(
            "Gemini API error: status=%d body=%s request=%s",
            status,
            err_body,
        ...
        return AIBackendError(
            f"Gemini API rejected request ({status}): {reason}",
            status=status,
        )
    ...
    def _error_from_stream_body(
        ...
        ai_logger.warning(
            "Gemini stream API error: status=%d body=%s",
            status,
            err_body,
        )
        reason = self._extract_error_reason(err_body)
        return AIBackendError(
            f"Gemini API rejected streaming request ({status}): {reason}",
            status=status,
        )
