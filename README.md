...
| [model-manager](#model-manager) | `model_manager` service (`/models` SPA page: installed models + browse the Hugging Face GGUF catalog with HF-native sort/search + a Recommended overlay) — gated on the Ollama backend being enabled | — (uses the `local_model_runtime` capability + `httpx`) | Intelligence |
...
The local-model **manager** — an in-app admin page for browsing, fit-checking, and pulling local models via Ollama (PRD: gilbert#32). The `/models` page lists the models currently installed in the runtime **and** lets you browse the Hugging Face GGUF catalog to find more. Hardware-fit verdicts and one-click pull are later slices that build on this.
