"""Tests for QwenAI backend — message translation, response parsing, streaming."""

from unittest.mock import AsyncMock, MagicMock

import pytest
from gilbert_plugin_qwen.qwen_ai import QwenAI
...
async def test_initialize_requires_api_key(backend: QwenAI) -> None:
    with pytest.raises(ValueError, match="api_key"):
        await backend.initialize({})


async def test_initialize_creates_client(backend: QwenAI) -> None:
    await backend.initialize({"api_key": "sk-test"})
    assert backend._client is not None
    await backend.close()


async def test_initialize_custom_model(backend: QwenAI) -> None:
    await backend.initialize({"api_key": "sk-test", "model": "qwen-turbo"})
    assert backend._model == "qwen-turbo"
    await backend.close()


async def test_initialize_default_base_url_is_dashscope_intl(backend: QwenAI) -> None:
    """The default base URL must point at the international DashScope
    endpoint so the backend works out of the box for users outside
    mainland China."""
    await backend.initialize({"api_key": "sk-test"})
    assert backend._client is not None
...
async def test_initialize_custom_base_url(backend: QwenAI) -> None:
...
async def test_close_clears_client(backend: QwenAI) -> None:
    await backend.initialize({"api_key": "sk-test"})
    await backend.close()
...
def test_available_models_filtered_by_enabled() -> None:
    backend = QwenAI()
    backend._enabled_models = ["qwen-plus"]
...
def test_backend_config_params_includes_enabled_toggle() -> None:
    ...
    backends entirely."""
    params = QwenAI.backend_config_params()
...
def test_build_messages_user_with_image_attachment() -> None:
    """qwen-vl models accept image_url content parts with base64 data URLs,
    matching OpenAI's vision request shape."""
    backend = QwenAI()
    messages = [
        Message(
...
def test_build_messages_user_with_document_renders_as_text_stub() -> None:
    """Qwen (like OpenAI) can't ingest PDFs natively over the Chat
    Completions endpoint, so document attachments become a text stub
    pointing at the workspace tools."""
    backend = QwenAI()
    messages = [
        Message(
            role=MessageRole.USER,
...
def test_build_messages_splits_slash_command_combined_row() -> None:
    ...
    split into an assistant(tool_calls) + N tool-role rows."""
    backend = QwenAI()
    messages = [
...
def test_build_tools() -> None:
    ...
    result = QwenAI._build_tools(tools)
    assert len(result) == 1
...
def test_build_request_body_uses_max_tokens_not_max_completion_tokens() -> None:
    """DashScope's compatible-mode endpoint accepts the standard
    ``max_tokens`` field — unlike the OpenAI plugin, we don't need to
    use ``max_completion_tokens`` because Qwen has no ``o``-series
    reasoning-model equivalent requiring it."""
    backend = QwenAI()
    backend._model = "qwen-plus"
...
def test_parse_null_content_with_tool_calls() -> None:
    """DashScope — like OpenAI — returns ``"content": null`` alongside
    tool_calls. Treat that as empty text."""
    backend = QwenAI()
    data = {
        "choices": [
            {
...
async def test_generate_calls_api(backend: QwenAI) -> None:
    await backend.initialize({"api_key": "sk-test"})
...
async def test_generate_raises_ai_backend_error_on_http_error(
    ...
    """A 4xx response should surface DashScope's error.message, not opaque HTTP text."""
    await backend.initialize({"api_key": "sk-test"})
...
async def test_generate_raises_when_not_initialized(backend: QwenAI) -> None:
    request = AIRequest(messages=[Message(role=MessageRole.USER, content="Test")])
...
async def test_generate_stream_text_deltas_and_complete(backend: QwenAI) -> None:
    """DashScope streams text via ``delta.content`` chunks — identical
    to OpenAI's SSE format."""
    await backend.initialize({"api_key": "sk-test"})
    assert backend._client is not None
...
async def test_generate_stream_tool_call_reassembly(backend: QwenAI) -> None:
    """Tool-call deltas arrive with ``id`` + ``name`` on the first chunk
    and streaming ``arguments`` on later chunks — same shape as OpenAI."""
    await backend.initialize({"api_key": "sk-test"})
    assert backend._client is not None
...
async def test_generate_stream_raises_on_http_error(backend: QwenAI) -> None:
