"""Qwen AI backend — AI via Alibaba DashScope's OpenAI-compatible API.

DashScope exposes an OpenAI-compatible Chat Completions endpoint at
``/compatible-mode/v1/chat/completions`` that accepts the same request
shape, streaming protocol, and tool-calling format as OpenAI. We speak
it directly over ``httpx`` rather than pulling in Alibaba's SDK, so the
backend has zero third-party deps beyond core Gilbert.
"""
...
# International DashScope endpoint — reachable outside mainland China.
# Users inside China should override to
# ``https://dashscope.aliyuncs.com/compatible-mode/v1`` via the config.
...
_AVAILABLE_MODELS = [
    ModelInfo(
        id="qwen3-max",
        name="Qwen3 Max",
        description="Latest flagship — strongest reasoning and tool use.",
    ),
    ModelInfo(
        id="qwen-max",
        name="Qwen Max",
        description="Flagship general-purpose model — strong reasoning and tool use.",
    ),
    ModelInfo(
        id="qwen-plus",
        name="Qwen Plus",
        description="Balanced mid-tier — good quality at much lower cost than Max.",
    ),
    ModelInfo(
        id="qwen-turbo",
        name="Qwen Turbo",
        description="Fast and cheap — ideal for simple tasks and high-volume use.",
    ),
    ModelInfo(
        id="qwen2.5-72b-instruct",
        name="Qwen2.5 72B Instruct",
        description="Open-weight 72B dense model hosted by DashScope.",
    ),
    ModelInfo(
        id="qwen2.5-32b-instruct",
        name="Qwen2.5 32B Instruct",
        description="Open-weight 32B dense model — good quality/speed trade-off.",
    ),
    ModelInfo(
        id="qwen2.5-coder-32b-instruct",
        name="Qwen2.5 Coder 32B",
        description="Code-specialized 32B model — strong at code generation and review.",
    ),
    ModelInfo(
        id="qwq-32b-preview",
        name="QwQ 32B Preview",
        description="Reasoning-focused preview model — stronger for math and planning.",
    ),
    ModelInfo(
        id="qwen-vl-max",
        name="Qwen-VL Max",
        description="Vision-language flagship — image understanding and multimodal chat.",
    ),
    ModelInfo(
        id="qwen-vl-plus",
        name="Qwen-VL Plus",
        description="Vision-language mid-tier — cheaper image understanding.",
    ),
]


class QwenAI(AIBackend):
    """AI backend using DashScope's OpenAI-compatible Chat Completions API."""
    ...
    def backend_config_params(cls) -> list[ConfigParam]:
        ...
        return [
            ...
            ConfigParam(
                ...
                description="DashScope API key (``sk-…``).",
                sensitive=True,
            ),
            ConfigParam(
                key="base_url",
                type=ToolParameterType.STRING,
                description=(
                    "DashScope API base URL. Leave at the default for the "
                    "international endpoint; switch to "
                    "``https://dashscope.aliyuncs.com/compatible-mode/v1`` "
                    "for the mainland-China endpoint, or point at a local "
            ...
            ConfigParam(
                ...
                description=(
                    ...
                    "``max_tokens`` to the DashScope compatible-mode endpoint."
                ),
                default=8192,
            ),
            ConfigParam(
    ...
    def backend_actions(cls) -> list[ConfigAction]:
        return [
            ConfigAction(
                ...
                description=(
                    "Send a tiny 'hi' message to the Qwen API to verify the API key and model."
                ),
            ),
        ]
    ...
    async def _action_test_connection(self) -> ConfigActionResult:
        if self._client is None:
            return ConfigActionResult(
                status="error",
                message="Qwen backend is not initialized — save settings first.",
            )
        try:
            request = AIRequest(
        ...
        return ConfigActionResult(
            status="ok",
            message=f"Connected to Qwen (model: {response.model}).",
        )
    ...
    async def initialize(self, config: dict[str, Any]) -> None:
        ...
        if not api_key:
            raise ValueError("QwenAI requires 'api_key' in config")
        ...
        logger.info("Qwen AI backend initialized (model=%s)", self._model)
    ...
    async def generate(self, request: AIRequest) -> AIResponse:
        if self._client is None:
            raise RuntimeError("QwenAI not initialized")
        ...
        ai_logger.debug(
            "Qwen response: finish_reason=%s usage=%s",
    ...
    async def generate_stream(
        ...
        """Stream DashScope SSE chunks as provider-neutral ``StreamEvent``s.

        DashScope's compatible-mode endpoint emits the same OpenAI SSE
        frame shape, so the parsing logic mirrors the OpenAI plugin:
        ...
        ai_logger.debug(
            "Qwen stream request: model=%s messages=%d",
            self._model,
            len(body["messages"]),
        )
        ...
        ai_logger.debug(
            "Qwen stream response: finish_reason=%s usage=%s",
            finish_reason_raw,
    ...
    def _build_messages(
        ...
        DashScope accepts OpenAI's message shape verbatim: the system
        prompt rides as the first ``{"role": "system"}`` message, tool
        results become ``{"role": "tool", "tool_call_id": ...}`` rows,
        and assistant tool calls live on the assistant message's
        ``tool_calls`` array with arguments encoded as a JSON string.
        """
        ...
        for msg in messages:
            ...
            if msg.role == MessageRole.USER:
                ...
                # DashScope (like OpenAI) requires the tool results to
    ...
    def _build_user_message(self, msg: Message) -> dict[str, Any]:
        ...
        Order mirrors the OpenAI backend: images, documents (as text
        stubs — only qwen-vl models accept images, and neither accepts
        PDFs natively), text attachments, opaque file stubs, then the
        user's typed prompt.
        """
    ...
    def _encode_tool_call(tc: ToolCall) -> dict[str, Any]:
        """DashScope (OpenAI-compat) expects ``tool_calls[i].function.arguments``
        as a JSON string, not a parsed dict."""
        return {
            "id": tc.tool_call_id,
            "type": "function",
            "function": {
    ...
    def _error_from_response(
        ...
        try:
            ...
            err_body = resp.text
        ai_logger.warning(
            "Qwen API error: status=%d body=%s request=%s",
            status,
            err_body,
        ...
        return AIBackendError(
            f"Qwen API rejected request ({status}): {reason}",
            status=status,
        )
    ...
    def _error_from_stream_body(
        ...
        ai_logger.warning(
            "Qwen stream API error: status=%d body=%s",
            status,
            err_body,
        )
        reason = self._extract_error_reason(err_body)
        return AIBackendError(
            f"Qwen API rejected streaming request ({status}): {reason}",
            status=status,
        )
