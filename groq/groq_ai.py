"""Groq AI backend — AI via the Groq OpenAI-compatible API.

Groq runs open-weight models (Llama, Qwen, Mixtral, DeepSeek distills)
on their custom LPU hardware and exposes an OpenAI-compatible Chat
Completions endpoint at ``https://api.groq.com/openai/v1/chat/completions``.
Main selling point is inference latency — tokens/second is multiples
higher than GPU-hosted providers. We speak it directly over ``httpx``.
"""
...
_AVAILABLE_MODELS = [
    ModelInfo(
        ...
        description="Meta's flagship 70B chat model — strong tool use and reasoning.",
    ),
    ModelInfo(
        id="llama-3.1-8b-instant",
        name="Llama 3.1 8B Instant",
        description="Small + extremely fast — ideal for high-volume simple tasks.",
    ),
    ModelInfo(
        id="qwen-2.5-32b",
        name="Qwen 2.5 32B",
        description="Alibaba's 32B open-weight chat model hosted on Groq.",
    ),
    ModelInfo(
        id="deepseek-r1-distill-llama-70b",
        name="DeepSeek R1 Distill Llama 70B",
        description="Reasoning-tuned Llama 70B distilled from DeepSeek R1.",
    ),
    ModelInfo(
        id="gemma2-9b-it",
        name="Gemma 2 9B",
        description="Google's open-weight 9B instruction-tuned model.",
    ),
]


class GroqAI(AIBackend):
    """AI backend using Groq's OpenAI-compatible Chat Completions API."""
    ...
    def backend_config_params(cls) -> list[ConfigParam]:
        ...
        return [
            ...
            ConfigParam(
                ...
                type=ToolParameterType.STRING,
                description=(
                    "API base URL. Leave at the default for Groq's hosted "
            ...
            ConfigParam(
                ...
                description=(
                    ...
                    "``max_tokens`` to the Groq endpoint."
                ),
                default=8192,
            ),
            ConfigParam(
    ...
    def backend_actions(cls) -> list[ConfigAction]:
        return [
            ConfigAction(
                ...
                description=(
                    "Send a tiny 'hi' message to the Groq API to verify the API key and model."
                ),
            ),
        ]
    ...
    async def _action_test_connection(self) -> ConfigActionResult:
        if self._client is None:
            return ConfigActionResult(
                status="error",
                message="Groq backend is not initialized — save settings first.",
            )
        try:
            request = AIRequest(
        ...
        return ConfigActionResult(
            status="ok",
            message=f"Connected to Groq (model: {response.model}).",
        )
    ...
    async def initialize(self, config: dict[str, Any]) -> None:
        ...
        if not api_key:
            raise ValueError("GroqAI requires 'api_key' in config")
        ...
        logger.info("Groq AI backend initialized (model=%s)", self._model)
    ...
    async def generate(self, request: AIRequest) -> AIResponse:
        if self._client is None:
            raise RuntimeError("GroqAI not initialized")
        ...
        ai_logger.debug(
            "Groq response: finish_reason=%s usage=%s",
    ...
    async def generate_stream(
        ...
        """Stream Groq SSE chunks as provider-neutral ``StreamEvent``s.

        Groq's endpoint emits OpenAI-shape SSE frames, so parsing
        ...
        ai_logger.debug(
            "Groq stream request: model=%s messages=%d",
            self._model,
            len(body["messages"]),
        )
        ...
        ai_logger.debug(
            "Groq stream response: finish_reason=%s usage=%s",
            finish_reason_raw,
    ...
    def _build_messages(
        ...
        for msg in messages:
            ...
            if msg.role == MessageRole.USER:
                ...
                # Groq (like OpenAI) requires the tool results to
    ...
    def _build_user_message(self, msg: Message) -> dict[str, Any]:
        ...
        Groq's current chat models don't accept native image
        attachments, so every attachment becomes a text stub pointing
        the model at the workspace tools.
        """
    ...
    def _error_from_response(
        ...
        try:
            ...
            err_body = resp.text
        ai_logger.warning(
            "Groq API error: status=%d body=%s request=%s",
            status,
            err_body,
        ...
        return AIBackendError(
            f"Groq API rejected request ({status}): {reason}",
            status=status,
        )
    ...
    def _error_from_stream_body(
        ...
        ai_logger.warning(
            "Groq stream API error: status=%d body=%s",
            status,
            err_body,
        )
        reason = self._extract_error_reason(err_body)
        return AIBackendError(
            f"Groq API rejected streaming request ({status}): {reason}",
            status=status,
        )
